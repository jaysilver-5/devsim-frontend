...
export default function useWorkspaceSession(
  ...
  const cleanupSocketListeners = useCallback(() => {
    ...
    socket.off(WsEvent.SESSION_STATUS);
  }, []);
  ...
  const bindSocketListeners = useCallback(() => {
    ...
    socket.on(
      ...
      (data: Partial<WorkspaceSessionData> & { status?: string; currentTicketSeq?: number }) => {
        ...
        setSession((prev) => {
          ...
          return {
            ...
            ...(typeof data.currentTicketSeq === "number"
              ? { currentTicketSeq: data.currentTicketSeq }
              : {}),
          };
        });
      }
