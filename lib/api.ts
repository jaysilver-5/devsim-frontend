...
const standup = {
  ...
  /** Lite version for OpenAI Realtime path — no TTS, 2-3s faster per turn. */
  completeTurnTextLite: (
    standupId: string,
    turnNumber: number,
    text: string,
    token?: string,
    ...
    request(`/standup/${standupId}/turn/${turnNumber}/complete-text-lite`, {
      method: "POST",
      body: { text },
      token: token || undefined,
      retryToken: retryToken || undefined,
...
const realtime = {
  createStandupClientSecret: (
    sessionId: string,
    standupNumber: number,
    token: string,
    retryToken?: string
  ) =>
    request(`/realtime/standup/${sessionId}/${standupNumber}/client-secret`, {
      method: "POST",
      token,
      retryToken,
    }),
