...
type StandupState = "INTRO" | "PLAYING" | "RECORDING" | "PROCESSING" | "FOLLOW_UP" | "COMPLETE";
...
export function StandupOverlay({
  ...
  const startStandup = useCallback(async () => {
    try {
      setState("PLAYING");
      const result = (await api.standup.start(sessionId, standupNumber, token)) as any;
      setStandupId(result.standupId || result.id);
      setCurrentPmText(result.pmQuestion || result.question || "");
      setCurrentPmAudio(result.pmAudioUrl || null);
      setTurns([{ pmQuestion: result.pmQuestion || result.question || "" }]);

      if (result.pmAudioUrl && audioRef.current) {
        audioRef.current.src = result.pmAudioUrl;
        audioRef.current.play().catch(() => {});
      } else {
        // No audio, go straight to recording/text
        setTimeout(() => checkMic(), 1500);
      }
    } catch (err: any) {
      setError(err.message || "Failed to start standup");
      setState("INTRO");
    }
  }, [sessionId, standupNumber, token]);
  ...
  const checkMic = useCallback(async () => {
    try {
      ...
      setMicAllowed(false);
      setState("RECORDING"); // Will show text fallback
    }
  }, []);
  ...
  const startRecording = useCallback(async () => {
    ...
    try {
      ...
      // Silence detection: stop after 15s max
  ...
  // ── Complete turn (send audio or text) ────────────
  const completeTurn = useCallback(async (audioBase64?: string) => {
    if (!standupId) return;
    setState("PROCESSING");
    ...
    try {
      ...
      if (result.isComplete) {
        ...
        // Follow-up
        setTurnNumber((n) => n + 1);
        setCurrentPmText(result.pmFollowUp || result.pmQuestion || "");
        setCurrentPmAudio(result.pmAudioUrl || null);
        setTextInput("");

        setTurns((prev) => [...prev, { pmQuestion: result.pmFollowUp || "" }]);
  ...
  }, [standupId, turnNumber, token, currentPmText, textInput, onComplete, checkMic]);
    ...
    <div className="fixed inset-0 z-[100] bg-ds-base/95 backdrop-blur-md flex items-center justify-center">
      ...
      <div className="w-full max-w-lg px-6">
        ...
        <div className="text-center mb-8">
          ...
          <h2 className="text-lg font-semibold text-ds-text">Standup with Sarah Chen</h2>
          <p className="text-sm text-ds-text-dim mt-1">
            {state === "INTRO" ? "Ready to check in on your progress?"
              : state === "PLAYING" || state === "FOLLOW_UP" ? "Sarah is speaking..."
              : state === "RECORDING" ? "Your turn"
        ...
        <div className="flex justify-center gap-1.5 mb-6">
          {Array.from({ length: 4 }, (_, i) => (
            <div key={i} className={`w-2 h-2 rounded-full transition-colors ${
              i < turnNumber ? "bg-ds-primary" : i === turnNumber - 1 ? "bg-ds-primary animate-pulse" : "bg-ds-border"
            }`} />
          ))}
        </div>
        ...
        <div className="min-h-[200px]">
          ...
          {(state === "PLAYING" || state === "FOLLOW_UP") && (
            <div className="p-5 rounded-xl bg-ds-surface border border-ds-border">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-[11px] font-semibold text-persona-sarah">Sarah Chen</span>
              </div>
              <p className="text-sm text-ds-text-secondary leading-relaxed">{currentPmText}</p>
            </div>
          )}
          ...
          {state === "RECORDING" && (
            <div className="space-y-4">
              ...
              <div className="p-4 rounded-lg bg-ds-surface/50 border border-ds-border">
                <div className="text-[10px] font-semibold text-persona-sarah mb-1.5">Sarah asked:</div>
                <p className="text-[12px] text-ds-text-muted leading-relaxed">{currentPmText}</p>
              </div>
              ...
              {micAllowed ? (
                <div className="text-center">
                  ...
                  <p className="text-xs text-ds-text-dim mt-2">
                    {recording ? "Click to stop recording" : "Click to start speaking"}
                  </p>
                </div>
              ) : (
                <div className="space-y-2">
                  <p className="text-[11px] text-ds-text-faint text-center">Microphone unavailable. Type your response:</p>
          ...
          {state === "PROCESSING" && (
            <div className="text-center py-8">
              ...
              <p className="text-sm text-ds-text-dim">Sarah is thinking about your response...</p>
            </div>
          )}
          ...
          {state === "COMPLETE" && (
            <div className="text-center py-4">
              ...
              <h3 className="text-lg font-semibold text-ds-text mb-2">Standup Complete</h3>
              <p className="text-sm text-ds-text-dim mb-1">Great check-in! Returning to workspace...</p>

              {/* Show turn scores if available */}
              {turns.some((t) => t.scores) && (
                <div className="mt-4 space-y-1">
                  {turns.filter((t) => t.scores).map((t, i) => (
                    <div key={i} className="flex items-center justify-center gap-2 text-xs text-ds-text-faint">
                      Turn {i + 1}:
                      {Object.entries(t.scores!).map(([k, v]) => (
                        <span key={k} className="text-ds-text-muted">{k}: <strong className={v >= 7 ? "text-ds-success" : v >= 5 ? "text-ds-warning" : "text-ds-danger"}>{v}/10</strong></span>
                      ))}
                    </div>
                  ))}
                </div>
        ...
        {turns.length > 1 && state !== "COMPLETE" && (
          <div className="mt-6 border-t border-ds-border pt-4">
            <div className="text-[10px] text-ds-text-faint uppercase tracking-wider mb-2">Conversation</div>
            <div className="space-y-2 max-h-32 overflow-y-auto">
              {turns.slice(0, -1).map((turn, i) => (
                <div key={i} className="text-[11px]">
                  <div className="text-persona-sarah font-medium">Sarah: <span className="text-ds-text-dim font-normal">{turn.pmQuestion}</span></div>
                  {turn.devResponse && (
                    <div className="text-ds-primary-muted font-medium mt-0.5">You: <span className="text-ds-text-dim font-normal">{turn.devResponse}</span></div>
                  )}
                </div>
              ))}
            </div>
